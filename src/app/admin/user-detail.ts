...
import { ASSIGNABLE_ROLES, STATUS_LABEL, STATUS_TONE, type AdminUserSummary } from './admin-user.model';
...
export class AdminUserDetail {
  ...
  protected async assignRole(user: AdminUserSummary, role: string): Promise<void> {
    ...
    try {
      await firstValueFrom(this.http.put<void>(`${ADMIN_USERS_ENDPOINT}/${user.userId}/roles/${role}`, {}));
  ...
  protected async revokeRole(user: AdminUserSummary, role: string): Promise<void> {
    this.pendingRole.set(role);
    try {
      await firstValueFrom(this.http.delete<void>(`${ADMIN_USERS_ENDPOINT}/${user.userId}/roles/${role}`));
